# Slice 1 — F0: Environment & `mbd_demo.slx`
...
- Build `mbd_demo.slx`: the **same** mass-spring-damper system as the first MATLAB prompt, but as a block diagram. Log the output signal explicitly with a **To Workspace** block so it is trivial to read (does not depend on `out.yout` structure).
...
3. Save as `mbd_demo.slx` at the repo root
